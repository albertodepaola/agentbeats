...
from .utils.deploy.deploy import (
    _deploy_current_terminal,
    _deploy_separate_terminals,
    _deploy_tmux,
)
...
def _check_environment():
    ...
    if frontend_node_modules.exists():
        ...
        issues.append(
            "Frontend dependencies not installed (run: agentbeats install_frontend)"
        )
...
def _run_frontend(
    ...
    if not frontend_dir.exists():
        ...
        print(
            f"Available frontend directories: {list((current_dir / 'frontend').glob('*'))}"
        )
        sys.exit(1)
    ...
    if not (frontend_dir / "node_modules").exists():
        print(
            f"Error: Frontend dependencies not installed for {webapp_version}. Run `agentbeats install_frontend --webapp_version {webapp_version}` to install them."
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "Make sure Node.js and npm are installed and frontend dependencies are installed."
        )
...
def _run_backend(
    ...
    def start_backend():
        ...
        try:
            ...
            if not supabase_auth:
                ...
                print(
                    "🚀 Development mode enabled - authentication will be bypassed for all API calls"
                )
    ...
    def start_mcp():
        ...
        try:
            ...
            print(
                "Make sure the MCP server script is available and dependencies are installed."
            )
...
def _run_openenv_eval(
    ...
    logging.getLogger("agentbeats").setLevel(logging.DEBUG)
    # logging.getLogger("agents").setLevel(logging.DEBUG)
    # logging.getLogger('openai').setLevel(logging.DEBUG)
    print("Debug logging enabled for agentbeats, agents, and openai modules\n")
...
def main():
    ...
    # run_agent command
    run_agent_parser = sub_parser.add_parser(
        "run_agent", help="Start an Agent from card"
    )
    ...
    run_agent_parser.add_argument(
        "--mcp", action="append", default=[], help="One or more MCP SSE server URLs"
    )

    # run command
    run_parser = sub_parser.add_parser(
        "run", help="Launch an Agent with controller layer"
    )
    ...
    load_scenario_parser = sub_parser.add_parser(
        "load_scenario",
        help="Launch a complete scenario from scenario.toml (agents, environment, etc.)",
    )
    load_scenario_parser.add_argument(
        "scenario_root", help="Path to scenario directory"
    )
    load_scenario_parser.add_argument(
        "--launch-mode",
        choices=["tmux", "separate", "current"],
        default="",
        help="Launching terminal; Will override scenario.toml's settings!",
    )
    load_scenario_parser.add_argument(
        "--register_agents",
        action="store_true",
        default=False,
        help="Register agents to backend (default: false)",
    )
    load_scenario_parser.add_argument(
        "--backend", help="Backend URL", default="http://localhost:9000"
    )

    # run_scenario command
    run_scenario_parser = sub_parser.add_parser(
        "run_scenario",
        help="Run a scenario from scenario.toml (requires frontend and backend to be running; eqivalant to `ab load_scenario` + register agent + start battle)",
    )
    ...
    run_scenario_parser.add_argument(
        "--frontend", help="Frontend URL", default="http://localhost:5173"
    )

    # run_backend command
    backend_parser = sub_parser.add_parser(
        "run_backend", help="Start the AgentBeats backend server"
    )
    backend_parser.add_argument("--host", default="localhost", help="Backend host")
    backend_parser.add_argument(
        "--backend_port", type=int, default=9000, help="Backend port (default: 9000)"
    )
    backend_parser.add_argument(
        "--mcp_port", type=int, default=9001, help=f"MCP port (default: 9001)"
    )
    backend_parser.add_argument(
    ...
    backend_parser.add_argument(
        "--supabase_auth",
        action="store_true",
        help="Enable Supabase authentication (default: use dev login mode)",
    )
    backend_parser.add_argument(
        "--public_url",
        help="Public URL for backend (e.g., http://yourdomain.com:9000).",
    )

    # run_frontend command
    frontend_parser = sub_parser.add_parser(
        "run_frontend", help="Start the AgentBeats frontend server"
    )
    ...
    frontend_parser.add_argument(
        "--host", default="localhost", help="Frontend host (default: localhost)"
    )
    frontend_parser.add_argument(
        "--frontend_port", type=int, default=5173, help="Frontend port (default: 5173)"
    )
    frontend_parser.add_argument(
        "--webapp_version",
        default="webapp-v2",
        help="Frontend webapp version to run (default: webapp)",
    )
    frontend_parser.add_argument(
        "--backend_url",
        help="Backend URL for API proxy (required for dev and preview modes)",
    )
    frontend_parser.add_argument(
        "--supabase_auth",
        action="store_true",
    ...
    frontend_install_parser = sub_parser.add_parser(
        "install_frontend", help="Install frontend dependencies"
    )
    frontend_install_parser.add_argument(
        "--webapp_version",
        default="webapp-v2",
        help="Frontend webapp version to install (default: webapp-v2)",
    )

    # deploy command
    deploy_parser = sub_parser.add_parser(
        "deploy", help="Deploy complete AgentBeats stack (backend + frontend + MCP)"
    )
    ...
    deploy_parser.add_argument(
        "--mcp_port", type=int, default=9001, help="MCP server port (default: 9001)"
    )
    deploy_parser.add_argument(
        "--supabase_auth",
        action="store_true",
    ...
    openenv_eval_parser = sub_parser.add_parser(
        "run_openenv_eval", help="Evaluate an agent on OpenEnv environment"
    )
    openenv_eval_parser.add_argument(
        "--agent_card", required=True, help="Path to agent card TOML file"
    )
    openenv_eval_parser.add_argument(
        "--env",
        required=True,
    ...
    openenv_eval_parser.add_argument(
        "--model_type", default="openai", help="Model type (default: openai)"
    )
    openenv_eval_parser.add_argument(
        "--model_name", default="gpt-4o-mini", help="Model name (default: gpt-4o-mini)"
    )
    openenv_eval_parser.add_argument(
        "--output_dir", help="Directory to save results (default: ./eval_results)"
    )
    openenv_eval_parser.add_argument(
        "--tool", action="append", default=[], help="Additional tool files"
    )
    openenv_eval_parser.add_argument(
        "--mcp", action="append", default=[], help="MCP server URLs"
    )
    ...
    if args.cmd == "run_agent":
        ...
        if args.register_agents:
            if not args.backend:
                parser.error("--backend is required when --register_agents is used")
        ...
        _run_backend(
            ...
            reload=args.reload,
            supabase_auth=args.supabase_auth,
